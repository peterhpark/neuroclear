...
class DoubleDirDataset(BaseDataset):
    """
    Loads image volume datasets. The dataset is consisted of multiple 3D image sub-volumes.
    This dataset loads one volume each from the source and target datasets.
    """
    #TODO for Kaarjel's data, we need to load the target volume and only pushes 2D images. 
    ...
    def __init__(self, opt):
        ...
        self.A_path_len = len(self.A_paths)
       # self.A_img_shape = self.A_img_vol.shape

        self.B_paths = make_dataset(opt.data_target)
        self.B_path_len = len(self.B_paths)
    ...
    def __getitem__(self, index):
        A_path = self.A_paths[random.randint(0, self.A_path_len)]
        self.A_img_vol = io.imread(A_path)
        self.A_img_vol_rotated = self.A_img_vol # initialize it as not rotated. 

        B_path = self.B_paths[random.randint(0, self.B_path_len)]

        self.B_img_vol = io.imread(B_path)
        ...
        if self.validate:
            ...
            return {'src': A, 'src_paths': self.A_paths, 'tgt': B, 'tgt_paths': self.B_paths, 'gt': C, 'gt_paths': self.C_path}
            ...
            return {'src': A, 'src_paths': self.A_paths, 'tgt': B, 'tgt_paths': self.B_paths}
