...
class AxialToLateralGANMeiliModel(BaseModel):
    """
    ...
    This model is a successor to Tuisto; it adds a second path: rec (anisotropic) -> rec_2 (isotropic) to have another
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.criterionCycle_1 = torch.nn.L1Loss()
            self.criterionCycle_2 = torch.nn.L1Loss()
    ...
    def forward(self):
        ...
        self.rec_1 = self.netG_B(self.fake)  # G_B(G_A(A))
        self.rec_2 = self.netG_A(self.rec_1) # G_A (G_B(G_A(A))
    ...
    def backward_D_B_lateral(self):
        self.loss_D_B_lateral = self.backward_D_projection(self.netD_B_lateral, self.real_src, self.rec_1, self.lateral_axis,
    ...
    def backward_D_B_axial(self): # compares real_tgt axial slice image and fake axial slice image.
        ...
        self.loss_D_B_axial_1 = self.backward_D_projection(self.netD_B_axial, self.real_src, self.rec_1, self.axial_1_axis,
                                                           self.axial_1_axis)  # comparing YZ_original to YZ_reconstructed

        self.loss_D_B_axial_2 = self.backward_D_projection(self.netD_B_axial, self.real_src, self.rec_1, self.axial_2_axis,
    ...
    def backward_G(self):
        ...
        self.loss_G_B_lateral = self.criterionGAN(self.proj_f(self.rec_1, self.netD_B_lateral, self.lateral_axis),
                                                  True) * self.lambda_plane_target
        self.loss_G_B_axial = self.criterionGAN(self.proj_f(self.rec_1, self.netD_B_axial, self.axial_1_axis),
                                                True) * self.lambda_slice + \
                              self.criterionGAN(self.proj_f(self.rec_1, self.netD_B_axial, self.axial_2_axis),
                                                True) * self.lambda_slice

        self.loss_G_B = self.loss_G_B_lateral + self.loss_G_B_axial * 0.5

        # This model only includes forward cycle loss || G_B(G_A(A)) - A||
        self.loss_cycle_1 = self.criterionCycle_1(self.rec_1, self.real_src) * lambda_A
        self.loss_cycle_2 = self.criterionCycle_2(self.rec_2, self.fake) * lambda_A
        ...
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_1 + self.loss_cycle_2
