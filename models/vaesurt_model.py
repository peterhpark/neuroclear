...
class VAESurtModel(BaseModel):
    ...
    This model is a successor to Surt; it adds VAE element. 
    ...
    def forward(self):
        ...
        """
        self.z_encoded, self.mu, self.logvar = self.encode(self.real_tgt)
        self.fake = self.netG_A(self.real_src, self.z_encoded)  # G_A(A)

        
        self.z_encoded_fake = self.proj_f(self.fake, self.encode, self.lateral_axis)


        random_index = np.random.randint(0, self.z_encoded_fake.size(2))
        self.z_encoded_fake = self.z_encoded_fake[:, :, random_index, :, :]
        
        self.rec = self.netG_B(self.fake)  # G_B(G_A(A))
        self.fake_2 = self.netG_A(self.rec, self.z_encoded_fake) # fake version of the original fake volume 
