...
class AxialToLateralGANSnotraModel(BaseModel):
    """
    ...
    This model is a successor to Nep; we apply 2D consistency loss for lateral projections between the original and generated volume. 
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_lateral_preserve', type=float, default=1.0,
                                help='weight for preserving lateral projections')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A_lateral', 'D_A_axial', 'G_A', 'G_A_lateral', 'G_A_axial', 'cycle',
                           'D_B_lateral', 'D_B_axial', 'G_B', 'G_B_lateral', 'G_B_axial', 'lateral_preserve_srctofake', 'lateral_preserve_faketorec']
        ...
        self.lambda_lateral_preserve = opt.lambda_lateral_preserve
        ...
        if self.isTrain:
            ...
            self.criterionLateralPreserve = torch.nn.L1Loss()
    ...
    def get_lateral_preserve_loss(self, source, fake, depth):
        """
        Calculate the loss to preserve the lateral projection information 
        """
        start_index = np.random.randint(0, self.num_slice - depth)  
        source_proj = torch.max(source[:, :, start_index:start_index + depth, :, :], 2)[0]
        fake_proj = torch.max(fake[:, :, start_index:start_index + depth, :, :], 2)[0]

        loss_lateral_preserve = self.criterionLateralPreserve(source_proj, fake_proj)

        return loss_lateral_preserve
    ...
    def backward_G(self):
        ...
        self.loss_lateral_preserve_srctofake = self.lambda_lateral_preserve * self.get_lateral_preserve_loss(self.real_src, self.fake, self.projection_depth)
        self.loss_lateral_preserve_faketorec =  self.lambda_lateral_preserve * self.get_lateral_preserve_loss(self.fake, self.rec, self.projection_depth)
        ...
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle + self.loss_lateral_preserve_srctofake + self.loss_lateral_preserve_faketorec 
