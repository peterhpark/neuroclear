...
class AxialToLateralGANForsetiModel(BaseModel):
    """
    ...
    This model is a successor to Tuisto; it adds additional projection loss through the whole image dimension.
                                        Furthermore, it keeps a memory bank of previous large-scale projections.
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--projection_depth', type=int, default=80,
                                help='depth for maximum intensity projections. ')
            parser.add_argument('--min_projection_depth', type=int, default=30,
                                help='minimum depth for maximum intensity projections. ')

            parser.add_argument('--pool_size', type=int, default=1,
                                help=' number of images to save in an image buffer to keep history for discriminators  ')

            parser.add_argument('--regularization_weight', type =float, default=0.01, help = 'Weight for the L1 regularization loss between the original and generated volume.')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A_lateral', 'D_A_axial', 'G_A', 'G_A_lateral', 'G_A_axial', 'cycle',
                           'D_B_lateral', 'D_B_axial', 'G_B', 'G_B_lateral', 'G_B_axial', 'reg']
        ...
        self.regularization_weight = opt.regularization_weight
        ...
        if self.isTrain:
            ...
            self.criterionRegularizerL1 = torch.nn.L1Loss()

            self.fake_pool_lateral = ImagePool(opt.pool_size)
            self.fake_pool_axial = ImagePool(opt.pool_size)
    ...
    def backward_D_projection(self, netD, real, fake, slice_axis_real, slice_axis_fake, is_A):
        ...
        # Real
        pred_real = self.proj_f(real, netD, slice_axis_real, use_buffer=False)
        pred_fake = self.proj_f(fake.detach(), netD, slice_axis_fake, use_buffer=is_A)
    ...
    def backward_D_A_lateral(self):
        self.loss_D_A_lateral = self.backward_D_projection(self.netD_A_lateral, self.real_tgt, self.fake, self.lateral_axis,
                                                      self.lateral_axis, is_A = True)  # comparing XY_original to XY_fake_MIP
    ...
    def backward_D_B_lateral(self):
        self.loss_D_B_lateral = self.backward_D_projection(self.netD_B_lateral, self.real_src, self.rec, self.lateral_axis,
                                                      self.lateral_axis, is_A = False)  # comparing XY_original to XY_reconstructed
    ...
    def backward_D_B_axial(self): # compares real_tgt axial slice image and fake axial slice image.
        ...
        self.loss_D_B_axial_1 = self.backward_D_projection(self.netD_B_axial, self.real_src, self.rec, self.axial_1_axis,
                                                      self.axial_1_axis, is_A = False)  # comparing YZ_original to YZ_reconstructed

        self.loss_D_B_axial_2 = self.backward_D_projection(self.netD_B_axial, self.real_src, self.rec, self.axial_2_axis,
                                                      self.axial_2_axis, is_A = False)  # comparing YZ_original to YZ_reconstructed
    ...
    def backward_G(self):
        ...
        self.loss_G_A_lateral = self.criterionGAN(self.proj_f(self.fake, self.netD_A_lateral, self.lateral_axis, use_buffer=True),
                                                  True) * self.lambda_plane_target

        self.loss_G_A_axial = self.criterionGAN(self.proj_f(self.fake, self.netD_A_axial, self.axial_1_axis, use_buffer=True),
                                                True) * self.lambda_slice + \
                              self.criterionGAN(self.proj_f(self.fake, self.netD_A_axial, self.axial_2_axis, use_buffer=True),
                                                True) * self.lambda_slice

        self.fake_mip_lateral = torch.max(self.fake, 2)[0]
        self.real_mip_lateral = torch.max(self.real_src, 2)[0]

        self.loss_reg = self.criterionRegularizerL1(self.fake_mip_lateral, self.real_mip_lateral) * self.regularization_weight

        self.loss_G_A = self.loss_G_A_lateral + self.loss_G_A_axial * 0.5 + self.loss_reg

        self.loss_G_B_lateral = self.criterionGAN(self.proj_f(self.rec, self.netD_B_lateral, self.lateral_axis, use_buffer=False),
                                                  True) * self.lambda_plane_target
        self.loss_G_B_axial = self.criterionGAN(self.proj_f(self.rec, self.netD_B_axial, self.axial_1_axis, use_buffer=False),
                                                True) * self.lambda_slice + \
                              self.criterionGAN(self.proj_f(self.rec, self.netD_B_axial, self.axial_2_axis, use_buffer=False),
                                                True) * self.lambda_slice
    ...
    def proj_f(self, input, function, slice_axis, use_buffer):
        input_volume = Volume(input, self.device)
        mip = input_volume.get_projection(self.projection_depth, slice_axis)

        if use_buffer:
            if slice_axis == 0: # Lateral images
                mip = self.fake_pool_lateral.query(mip) # image buffer to save previous lateral images
            else: # Axial images
                mip = self.fake_pool_axial.query(mip) # image buffer to save previous axial images
