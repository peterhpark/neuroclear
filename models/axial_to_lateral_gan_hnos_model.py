...
class AxialToLateralGANHnosModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_str', type=float, default=1e-1, help='weight for structural preservation loss')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A_lateral', 'D_A_axial', 'G_A', 'G_A_lateral', 'G_A_axial', 'cycle',
                           'D_B_lateral', 'D_B_axial', 'G_B', 'G_B_lateral', 'G_B_axial', 'str_preserve_A', 'str_preserve_B']
        ...
        self.visual_names = ['real_tgt', 'real_src', 'fake', 'fake_xy', 'fake_yz', 'rec']
        ...
        self.lambda_str = opt.lambda_str
        ...
        if self.isTrain:
            self.model_names = ['G_A_axial', 'G_A_lateral', 'G_B_axial', 'G_B_lateral', 'D_A_lateral', 'D_A_axial', 'D_B_lateral', 'D_B_axial']
        else:  # during test time, only load Gs
            self.model_names = ['G_A_axial', 'G_A_lateral', 'G_B_axial', 'G_B_lateral']
        ...
        if self.isTrain:
            ...
            self.criterionStructure = torch.nn.MSELoss()
            ...
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG_A_lateral.parameters(), self.netG_A_axial.parameters(),
                                                                self.netG_B_lateral.parameters(),self.netG_B_axial.parameters()),
    ...
    def forward(self):
        ...
        self.fake_xy= self.slice_f(self.real_src, self.netG_A_lateral, slice_axis=0)
        self.fake = torch.mean(torch.stack([self.fake_xz, self.fake_yz, self.fake_xy]), dim=0, keepdim=True).squeeze(dim=0)
        ...
        self.rec = torch.mean(torch.stack([self.rec_xz, self.rec_yz, self.rec_xy]), dim=0, keepdim=True).squeeze(dim=0)
    ...
    def backward_G(self):
        ...
        self.loss_str_preserve_A = self.criterionStructure(self.fake_xy, self.fake_xz) + \
            + self.criterionStructure(self.fake_yz, self.fake_xz) + self.criterionStructure(self.fake_xy, self.fake_yz)
        
        self.loss_str_preserve_B = self.criterionStructure(self.rec_xy, self.rec_xz) + \
            + self.criterionStructure(self.rec_yz, self.rec_xz) + self.criterionStructure(self.rec_xy, self.rec_yz)
        ...
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle +  self.lambda_str*(self.loss_str_preserve_A  +  self.loss_str_preserve_B) 
