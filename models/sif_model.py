...
class SifModel(BaseModel):
    ...
    This model is a successor to Sif; the model takes only slices from the target images.
    ...
    def backward_D_A_axial(self): # compares real_tgt XY slice image and fake axial MIP image.
        ...
        self.loss_D_A_axial_1 = self.backward_D_forwardpath(self.netD_A_axial, self.real_tgt, self.fake, self.axial_1_axis)  # comparing XY_original to YZ_fake

        self.loss_D_A_axial_2 = self.backward_D_forwardpath(self.netD_A_axial, self.real_tgt, self.fake, self.axial_2_axis)
