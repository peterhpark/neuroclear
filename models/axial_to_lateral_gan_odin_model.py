...
class AxialToLateralGANThorModel(BaseModel):
    ...
    This model is a successor to Thor; it only uses a 2D image for the target domain
    ...
    def backward_D_projection(self, netD, real, fake, slice_axis_real, slice_axis_fake):
        ...
        # Real
        pred_real = self.__apply_f(real, netD, slice_axis_real)
        pred_fake = self.__apply_f(fake.detach(), netD, slice_axis_fake)
    ...
    def backward_G(self):
        ...
        self.loss_G_A_lateral = self.criterionGAN(self.__apply_f(self.fake, self.netD_A_lateral, self.lateral_axis),
                                                  True) * self.lambda_plane_target

        self.loss_G_A_axial = self.criterionGAN(self.__apply_f(self.fake, self.netD_A_axial, self.axial_1_axis),
                                                True) * self.lambda_slice + \
                              self.criterionGAN(self.__apply_f(self.fake, self.netD_A_axial, self.axial_2_axis),
                                                True) * self.lambda_slice

        self.loss_G_A = self.loss_G_A_lateral + self.loss_G_A_axial * 0.5

        self.loss_G_B_lateral = self.criterionGAN(self.__apply_f(self.rec, self.netD_B_lateral, self.lateral_axis),
                                                  True) * self.lambda_plane_target
        self.loss_G_B_axial = self.criterionGAN(self.__apply_f(self.rec, self.netD_B_axial, self.axial_1_axis),
                                                True) * self.lambda_slice + \
                              self.criterionGAN(self.__apply_f(self.rec, self.netD_B_axial, self.axial_2_axis),
                                                True) * self.lambda_slice
    ...
    def __apply_f(self, input, function, slice_axis):
        img_dim = input.shape

        if len(img_dim) == 5: # 3D image
